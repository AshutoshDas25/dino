var dinoI, dino,ground,cloud,ran,cactus,desert,score=0,gamestate = "play";
function preload(){
...
function setup() {
  createCanvas(400, 400);
   dino=createSprite(100,280,50,80);
 ...
 ground=createSprite(200,350,800,10);
  ...
  ground2=createSprite(200,360,800,10);
...
//  cloud.scale=0.9;
  desert=new Group();
 air=new Group(); 
  game=createSprite(200,100,20,20);
  game.addAnimation("k",game39);
  game.scale=0.7;
  game.visible=false;
  retry=createSprite(200,130,20,20);
...
function draw() {
  ...
  if(ground.x<=0){
    ...
    ran=(Math.round(random(150,200)));
  //%(MODULUS);14%3=2,30%5=0;
  if(frameCount%60==0){
      cloud=createSprite(450,ran,20,20); 
    air.add(cloud);
  cloud.addAnimation("ok",cloud3);
  cloud.scale=0.9;
    cloud.velocityX=-3;
  ...
  //  console.log("c-",cloud.depth);
   ...
   if(frameCount%90==0){
  cactus=createSprite(450,320,10,60);
    cactus.velocityX=-3;
    cactus.lifetime=200;
    desert.add(cactus);
